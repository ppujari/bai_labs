...
MODEL_NAME = "sbert"
SEARCH_TYPE = "vector"
INDEX_RAW = "pp-vs-raw-sbert-docs-v1"       # stores original raw documents
INDEX_VECTOR = "pp-vs-sbert-active-ecodes-embeddings"  # stores documents with vector embeddings
INDEX_NAME =   "pp-vs-sbert-active-ecodes-embeddings"
...
IDENTIFIER_MAP = {
        ...
        '5382': 'Product Type',
        '4285': 'Activity',
        '5490':	'Raffle Eligible Product',
        '3699': 'Promotion Exclusion - ITEM',
        '5487': 'Exclusive To',
        '3961': 'WSC Ready',
        '404': 'Product Brand',
        '4426': 'VDC Eligible Flag',
        '3695': 'Web Country Of Origin',
        '2101': 'Gender by Age',
        '3453': 'NBA Player',
        '3446': 'EM-NFL Player',
        '3686': 'Product Buyable Flag',
        '4749': 'Custom Flag',
        '3449': 'EM-MLB Player',
        '3451': 'EM-NHL Player',
        '3674': 'Size Chart',
        '4289': 'DSG Product Title',
        '3700': 'Promotion Exclusion - ORDER',
        '4539': 'EM-Licensed Product Type',
        '3668': 'Full Image',
        '5429': 'Coming Soon End Date',
        '3468': 'Soccer Player',
        '3675': 'Product Displayable Flag',
        'None': ' '
    }
...
def prepare_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def flatten_row(row: pd.Series) -> str:
        ...
        for facet in facets:
            # Make sure each facet is a dict and has 'identifier' and 'value'
            if isinstance(facet, dict) and 'identifier' in facet and 'value' in facet:
                ...
                if key:
                    ...
                    info['None'] = (info.get('None', '') + " " + facet['value']).strip()

        # Join all keys dynamically except _id and None
        ordered_keys = ['Item'] + [k for k in info.keys() if k not in ('_id', 'Item', 'None')]
        result = '. '.join(f"{k}: {info[k]}" for k in ordered_keys if k in info)

        # Append 'None' value without key if it exists
        if 'None' in info:
            result += '. ' + info['None']
