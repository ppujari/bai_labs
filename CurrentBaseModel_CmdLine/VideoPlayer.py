...
class PyPlayer(tk.Frame):
    def __init__(self,vido, container, container_instance, title=None):
        tk.Frame.__init__(self, container_instance)
        self.container = container
        self.container_instance = container_instance
        self.initial_directory = pathlib.Path(os.path.expanduser("/home/hduser/Bai-labs/bai_labs-master2/Smoking_detection_Cmd_Line_v2/test1/"))
    ...
    def create_control_panel(self):
        ...
        predict=ttk.Button(control_panel, text="Predict", command=self.predict)
        Next=  ttk.Button(control_panel,text="Next",command=self.Next)
    ...
    def Next(self):
        ...
        l=len(self.vpth)
        if self.i==l:
            print("\n All videos are predicted \n\n")
            self.i=0

        self.vid=str(self.vpth[self.i])
        file =os.path.join(str(self.initial_directory),self.vid)
        if os.path.isfile(file):
            self.open()
    def predict(self):
        print(self.vid)
        pb.FrameCapture(str(self.vid))

    def open(self):
        """New window allowing user to select a file and play."""
        #file=self.initial_directory
        #file = askopenfilename(initialdir=self.vid,filetypes=(("Audio Video Interleave", "*.avi"),
    ...
    def play_film(self, file):
        ...
        print("\n  {}".format(self.vid))
        file_name=file
        self.Media = self.vlc_instance.media_new(str(file_name))
    ...
    def create_list_menu(self):
        ...
        #self.create_film_entry(self.vid)
...
class BaseTkContainer:
    def __init__(self):
        ...
        self.tk_instance.geometry("700x400+200+200") # default to 1080p
...
player = PyPlayer(vidir,root, root.tk_instance, title="pyplayer")
