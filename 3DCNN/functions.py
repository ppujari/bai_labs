...
def change_path(data_path):
    root = data_path
    catg=listdir(data_path)
    for f in catg:
        print(f)
        for filename in listdir(join(root, f)):
            print(filename)
            move(join(root, f, filename), join(root, filename))
        rmdir(join(root,f))
...
class Dataset_3DCNN(data.Dataset):
    ...
    def read_images(self, path, selected_folder, use_transform):
        ...
        for i in self.frames:
            #convert('L') makes the image grayscale
            image = Image.open(os.path.join(path, selected_folder, 'frame'+str(i-1)+'.jpg'.format(i))).convert('L')
            #the line below keeps the image as RGB
            #in channels will depend on one of the two (RGB/GrayscaleS)
            #image = Image.open(os.path.join(path, selected_folder, 'frame'+str(i-1)+'.jpg'.format(i)))
...
def Conv3d_final_prediction(model, device, loader):
    ...
    with torch.no_grad():
        for batch_idx, (X, y) in enumerate(tqdm(loader)):
            ...
            all_y_pred.extend(y_pred.numpy().tolist())
...
class CNN3D(nn.Module):
    def __init__(self, t_dim=120, img_x=90, img_y=120, drop_p=0.2, fc_hidden1=256, fc_hidden2=128, num_classes=2):
        ...
        #in channels 3 RGB (or 1 channel)?
        #in channels is made 1 since we make the image a grayscale one
        #if the images remains as RGB, the in_channels should be set to 3
        #but this later gives a dimensions exceeded error
