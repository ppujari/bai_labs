...
MODEL_NAME = "e5"
SEARCH_TYPE = "vector"
INDEX_VECTOR = "pp-vs-e5-llmd-embeddings-newprompt-uat-raw"
INDEX_NAME = "pp-vs-e5-llmd-embeddings-newprompt-uat-raw"
SIZE=4
...
def get_embedding_column(model_name: str) -> str:
    ...
    return "vs_sbert_llm_product_desc_embedding" if model_name.lower() == "sbert" else "vs_e5_llm_product_desc_embedding"
...
def run_query(es: Elasticsearch, query_text: str, size: int):
    ...
    knn_query_body = {
        ...
        "knn": {
            "field": search_field,
            "k": size,
            "num_candidates": max(size * 50, 1000),
...
def main():
    ...
    output_file = "matched_results_e5_50.csv"
    ...
    print(f"E5 average metrics: { {k: round(v, 2) for k, v in avg_sbert.items()} }")
