...
if __name__ == "__main__":
    ...
    if opt.resume_path:
        ...
        #    'D:/bailabs/cnn-lstm/test/Gunshot.avi')
        parent='D:/bailabs/cnn-lstm/'
        child='test_ns/'
        #cam=cv2.VideoCapture(os.path.join(parent,child))
        #cam=cv2.VideoCapture('D:/bailabs/cnn-lstm/test/Pirates_6_talk_h_nm_np1_fr_med_1.avi')
        for video_name in os.listdir(os.path.join(parent,child)):
            print(video_name)
            clip = []
            frame_count = 0
            cam=cv2.VideoCapture(os.path.join(parent,child,video_name))
            while True:
                ret, img = cam.read()
                if not ret:
                    break
