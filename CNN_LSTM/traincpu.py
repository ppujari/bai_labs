...
parser.add_argument('--fc-size', default=4096, type=int, help='size of fully connected layer before LSTM')					
#1024 or 4096 .... 1024 for resnet50, what for alexnet?
parser.add_argument('--epochs', default=15, type=int, help='manual epoch number')
...
def train(train_loader, model, criterion, optimizer, epoch):
	...
	for i, (inputs, target, _) in enumerate(train_loader):
		...
		#print(inputs)
		#print(len(inputs))
		input_var = [input.to(device) for input in inputs]
		...
		print('Epoch: [{0}][{1}/{2}]\t'
			...
			'Top2 {top5.val:.3f} ({top5.avg:.3f})'.format(
...
if __name__ == '__main__':
	...
	if os.path.exists(os.path.join(args.model, 'checkpoint.pth.tar')):
		...
		model = LSTMModel(original_model, args.arch,len(train_dataset.classes), args.lstm_layers, args.hidden_size, args.fc_size)
		#print(model)
		#model.cuda()
		cur_epoch = 0
