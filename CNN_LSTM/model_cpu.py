...
class LSTMModel(nn.Module):
	...
	def forward(self, inputs, hidden=None, steps=0):
		...
		#print(inputs)
		fs = torch.zeros(inputs[0].size(0), length, self.rnn.input_size).to(device)
		#print(fs)
		for i in range(length):
			f = self.features(inputs[i])
			#print(inputs[i])
			#print(f.shape)  #torch.Size([8, 256, 6, 6])
			f = f.view(f.size(0), -1) # getting reshaped here.
			#m1 x m2 error if above lined removed.
			#print(f.shape) #torch.Size([8, 9216])
			f = self.fc_pre(f)
			fs[:, i, :] = f
			#print(fs)
